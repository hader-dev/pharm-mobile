...
DeligateClient jsonToClient(Map<String, dynamic> json) {
  return DeligateClient(
    ...
    status: json['status'],
    buyerCompany: json['buyerCompany'] != null
        ? jsonToCompany(json['buyerCompany'])
        : Company.empty(),
  );
}
...
ResponseMyClients jsonToResponseMyClients(Map<String, dynamic> json) {
  ...
  List<DeligateClient> clients =
      clientsJson.map((clientJson) => jsonToClient(clientJson)).toList();
