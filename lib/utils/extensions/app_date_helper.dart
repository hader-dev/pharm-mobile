...
extension DateTimeHelper on DateTime {
  String get format => DateFormat("yyyy-MM-dd").add_Hm().format(toLocal());
  String get formatYMD => DateFormat("yyyy-MM-dd").format(toLocal());
  ...
  String getTimingAgo(AppLocalizations localization) {
    ...
    if (diff.inHours < 1) {
      return diff.inMinutes == 1 ? localization.minute_ago : localization.minutes_ago(diff.inMinutes);
    }

    if (diff.inDays < 1) {
      return diff.inHours == 1 ? localization.hour_ago : localization.hours_ago(diff.inHours);
    }

    if (diff.inDays == 1) return localization.yesterday;

    if (diff.inDays < 7) {
      return diff.inDays == 1 ? localization.day_ago : localization.days_ago(diff.inDays);
    }
    ...
    if (diff.inDays < 365) {
      ...
      return months == 1 ? localization.month_ago : localization.months_ago(months);
    }
