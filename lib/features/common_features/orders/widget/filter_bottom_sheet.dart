  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider.value(
          value: OrdersScreen.scaffoldKey.currentContext!
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: OrderStatus.values.map((status) {
                      final isSelected =
                          state.filters.status.contains(status.name);
                      return FilterChip(
                          label: Text(
                            status.name,
                            style: context
                          ...
                          onSelected: (selected) {
                            if (selected) {
                              context.read<OrdersCubit>().updatedFilters(
                                  state.filters.updateFilterList(
                                      OrderFiltersKeys.status,
                                      [...state.filters.status, status.name]));
                              return;
                            }
                            state.filters.status.removeWhere(
                          ...
                          backgroundColor:
                              AppColors.accent1Shade1.withAlpha(30));
                    }).toList(),
                  ),
                  ...
                  CustomTextField(
                    ...
                    isReadOnly: true,
                    initValue: state.filters.createdAtFrom.isNotEmpty
                        ? state.filters.createdAtFrom.first
                        : null,
                    validationFunc: (value) {
                      if (value != null &&
                        ...
                        final toDate =
                            DateTime.parse(state.filters.createdAtTo.first);
                    ...
                    onTap: () async {
                      ...
                      if (pickedDate != null) {
                        cubit.updatedFilters(state.filters.updateFilterList(
                            OrderFiltersKeys.createdAtFrom,
                            [pickedDate.formatYMD]));
                      }
                    },
                  ...
                  CustomTextField(
                    key: ValueKey(
                        "to-${state.filters.createdAtTo.isNotEmpty ? state.filters.createdAtTo.first : ''}"),
                    label: translation.to,
                    hintText: translation.date,
                    hintTextStyle:
                        context.responsiveTextTheme.current.body3Regular,
                    isReadOnly: true,
                    initValue: state.filters.createdAtTo.isNotEmpty
                        ? state.filters.createdAtTo.first
                        : null,
                    onTap: () async {
                      DateTime? pickedDate = await showDatePicker(
                          context: context,
                    ...
                    suffixIcon: state.filters.createdAtTo.isNotEmpty
                        ? InkWell(
                            onTap: () {
                              cubit.updatedFilters(state.filters
                                  .updateFilterList(
                                      OrderFiltersKeys.createdAtTo, []));
                            },
                            child: Icon(
                          ...
                          child: PrimaryTextButton(
                            ...
                            onTap: () {
                              if (formKey.currentState!.validate()) {
                                context
                                    .read<OrdersCubit>()
                                    .getOrders(filters: state.filters);
                                context.pop();
                              }
                            },
                          ),
